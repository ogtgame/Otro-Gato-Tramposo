...
const EXP2_BLACK = [
  // PEGÁ ACÁ TUS 15 CARTAS NEGRAS
  "No todas las chicas saben __________."
"En las películas para adultos, lo más difícil es __________."
"Lo que nunca falta en un casting porno es __________."
"Mi ex me dijo que yo era bueno en __________."
"Según Pornhub Argentina, lo más buscado es __________."
"Lo que toda actriz porno sueña con hacer es __________."
"El secreto para durar más de 3 minutos es __________."
"Lo que más excita a los directores de cine adulto es __________."
"Lo que ninguna mina admite que le gusta es __________."
"La posición que nadie logra bien en la vida real es __________."
"Mi fantasía secreta siempre incluye __________."
"El problema de follar con una estrella porno es __________."
...
const EXP2_WHITE = [
  ...
  "Hacer petes"
"Fingir el orgasmo"
"Un creampie sorpresa"
"El famoso 'solo la puntita'"
"Un gangbang de amigos"
"Usar el celular en el medio"
"Un facial en la primera cita"
"Gemir como en el porno"
"La posición del misionero con alma"
"Un threesome que termina en drama"
"Venirse en menos de 30 segundos"
"El 'no mires' mientras te la comes"
"Un blowjob con lágrimas"
"La cowgirl con celular en la mano"
"Un anal sin previo aviso"
"Gemir el nombre de la ex"
"El famoso 'me vengo adentro'"
"Un footjob profesional"
"Un rimjob sorpresa"
"Venirse en la cara sin preguntar"
"Hablar sucio en lunfardo"
"El clásico 'no te preocupes, me retiro'"
"Un deepthroat fallido"
"Fingir que te gusta el anal"
"El 'vamos a grabar para el recuerdo'"
"Un bukkake con amigos"
"La doble penetración emocional"
"Un squirt épico"
"Usar el lubricante sabor frutilla"
"Hablar como actriz porno"
"El 'mirá a cámara'"
"Un 69 que termina en risa"
...
const EXP3_BLACK = [
  // PEGÁ ACÁ TUS 15 CARTAS NEGRAS
  "Lo que más le gusta a los curas es __________."
"El verdadero milagro de la Virgen de Luján fue __________."
"Según los Testigos de Jehová, el fin del mundo va a empezar con __________."
"Lo que nunca falta en una misa es __________."
"El Papa Francisco confesó que su pecado favorito es __________."
"En el seminario lo que más se enseña es __________."
"Jehová fue testigo de __________."
"El secreto mejor guardado de la Iglesia es __________."
"Lo que todo cura sueña con hacer en el confesionario es __________."
"El verdadero motivo de la última cena fue __________."
"Lo que los Testigos de Jehová odian más que la sangre es __________."
"El pecado original en realidad fue __________."
...
const EXP3_WHITE = [
  // PEGÁ ACÁ TUS 35 CARTAS BLANCAS
  "Los menores de 12"
"Tocarle el pito al monaguillo"
"Pedófilos con sotana"
"Convertir agua en fernet"
"Un coro de nenes afinados"
"La plata de la colecta"
"Un judío con un Porsche"
"72 vírgenes que en realidad son 72 primos"
"El cura que se hace la paja en el confesionario"
"Un Testigo de Jehová que vota"
"La Virgen María haciendo un trío"
"Un rabino comiendo jamón crudo"
"El Papa fumando porro"
"La hostia con LSD"
"Los chicos del coro"
"Un milagro de plástico"
"Un cura con OnlyFans de pies"
"40 días y 40 noches de joda"
"La sangre de Cristo pero en tetrabrik"
"Un pastor evangélico con yate"
"El confesionario con glory hole"
"Una monja con tanga"
"Un imán comiendo asado"
"Un Testigo de Jehová en un after"
"El pecado de Onán pero con final feliz"
"Un cura que cobra en dólares"
"La resurrección de Maradona"
"Los niños del Vaticano"
"Un milagro que sale mal"
"La segunda venida de Cristo pero en el Obelisco"
"Un monje con un harem"
"Un imam bailando reggaetón"
...
function startTurnTimer(room) {
  ...
  room.turnTimer = setTimeout(() => {
    if (!rooms[room.code]) return; // sala ya no existe

    if (room.phase === 'playing') {
      // Auto-enviar carta al azar para jugadores que no enviaron
      const nonJudge = room.players.filter(p => !p.isJudge);
      nonJudge.forEach(p => {
        const alreadySubmitted = room.submissions.find(s => s.playerId === p.id);
        if (!alreadySubmitted && p.hand.length > 0) {
          ...
          // Quitar del hand real
          cards.forEach(card => {
            const idx = p.hand.indexOf(card);
            if (idx !== -1) p.hand.splice(idx, 1);
          });
          room.submissions.push({ playerId: p.id, playerName: p.name, cards, autoSubmitted: true });
        }
      });
      ...
      room.phase = 'judging';
      broadcastState(room);
      // Iniciar timer para el juez
      startTurnTimer(room);

    } else if (room.phase === 'judging') {
      // Auto-elegir respuesta al azar
      if (room.shuffledSubmissions.length > 0) {
        const randIdx = Math.floor(Math.random() * room.shuffledSubmissions.length);
        const submission = room.shuffledSubmissions[randIdx];
        ...
        if (winner) {
          ...
          room.phase = winner.score >= room.targetScore ? 'game_over' : 'round_result';
...
function getStateForPlayer(room, playerId) {
  ...
  // Tiempo restante del timer
  let timerSecondsLeft = TURN_TIMEOUT;
  if (room.turnStartedAt) {
    const elapsed = Math.floor((Date.now() - room.turnStartedAt) / 1000);
    timerSecondsLeft = Math.max(0, TURN_TIMEOUT - elapsed);
  }
  ...
  if (room.phase === 'game_over') {
    ...
    base.finalRanking = [...room.players].sort((a, b) => b.score - a.score)
...
function broadcastState(room) {
  room.players.forEach(p => {
    ...
    if (sock) sock.emit('game_state', getStateForPlayer(room, p.id));
  });
}
...
io.on('connection', (socket) => {
  ...
  socket.on('join_room', ({ roomCode, playerName }) => {
    ...
    if (room.phase !== 'lobby') {
      // Permitir reconexión por nombre
    ...
    if (room.players.length >= room.maxPlayers) {
      return socket.emit('error', { message: `La sala está llena (máximo ${room.maxPlayers})` });
    }
  ...
  socket.on('submit_cards', ({ roomCode, cards }) => {
    ...
    if (room.submissions.length >= nonJudge.length) {
      ...
      room.phase = 'judging';
      // Reiniciar timer para la fase de judging
      startTurnTimer(room);
    }
...
server.listen(PORT, () => {
  console.log(`O.G.T. server corriendo en puerto ${PORT}`);
});
